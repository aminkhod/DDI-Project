...
\begin{table}[h!]
	...
	\begin{tabular}{|c|c|c|c|}
		...
		\cline{1-4}
		&&&\\
		Predicted Non-interaction &$F_{N-Int}$ -- $cell_7$&$F_{N-Int}$ -- $cell_8$&$T_{N-Int}$ -- $cell_9$\\
		&&&\\
		\cline{1-4}
	\end{tabular}
	\newline
...
Table \ref{confusion_matrix_temp} shows the confusion matrix for the triple class case. In this mode, we have to redefine the definitions of TP, TF, TN, and TP, as well as accuracy, precision, recall, and F- measure. Base on Table \ref{confusion_matrix_temp} TP, TF, TN, and TP will define as Table \ref{TPFPFNTN}.
...
Now we can define the four evaluation criteria as the following:
